...
const ArtifactChatPanel: FC<ArtifactChatPanelProps> = ({ artifact }) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const systemPrompt = buildRefinementSystemPrompt(artifact)
      const userPrompt = buildRefinementUserPrompt(trimmedInput, artifact)

      // TODO: Connect to AI provider for actual refinement
    ...
    <Container>
      ...
      <InputArea>
        <StyledTextArea
          ...
          disabled={isRefining}
        />
        <SendButton type="primary" onClick={handleSend} disabled={!inputValue.trim() || isRefining} icon={<SendOutlined />}>
          {t('common.send')}
        </SendButton>
      </InputArea>
    </Container>
 ...
 */
function buildRefinementSystemPrompt(artifact: Artifact): string {
 ...
 */
function buildRefinementUserPrompt(request: string, artifact: Artifact): string {
