...
export function parseArtifacts(content: string): ParseResult {
  ...
  // Add remaining text after last artifact (only if we found any artifacts)
  if (artifacts.length > 0 && lastIndex < content.length) {
 ...
 */
export function updateArtifactContent(originalContent: string, artifactIdentifier: string, newContent: string): string {
