...
import { sanitizeArtifactDependencies } from './artifactDependencySanitizer'
...
const migrateConfig = {
  ...
  '189': (state: RootState) => {
    try {
      ...
      if (memoryEmbeddingApiClient) {
        state.memory.memoryConfig.embedderModel = getModel(
  ...
  '205': (state: RootState) => {
    try {
      if (state.settings?.artifacts?.react) {
        state.settings.artifacts.react.dependencies = sanitizeArtifactDependencies(
          state.settings.artifacts.react.dependencies,
          settingsInitialState.artifacts.react.dependencies
        )
      }
