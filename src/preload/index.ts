...
const api = {
  ...
  artifact: {
    startServer: (): Promise<{ success: boolean; port?: number; error?: string }> =>
      ipcRenderer.invoke(IpcChannel.Artifact_StartServer),
    stopServer: (): Promise<{ success: boolean; error?: string }> => ipcRenderer.invoke(IpcChannel.Artifact_StopServer),
    getServerStatus: (): Promise<{ running: boolean; port: number | null }> =>
      ipcRenderer.invoke(IpcChannel.Artifact_GetServerStatus),
    getState: (artifactId: string): Promise<Record<string, unknown>> =>
      ipcRenderer.invoke(IpcChannel.Artifact_GetState, artifactId),
    updateState: (artifactId: string, updates: Record<string, unknown>): Promise<{ success: boolean }> =>
      ipcRenderer.invoke(IpcChannel.Artifact_UpdateState, artifactId, updates),
    clearState: (artifactId: string): Promise<{ success: boolean }> =>
      ipcRenderer.invoke(IpcChannel.Artifact_ClearState, artifactId),
    onServerStatus: (
      callback: (payload: { event: 'started' | 'stopped'; port: number | null }) => void
    ): (() => void) => {
      const listener = (_: Electron.IpcRendererEvent, payload: { event: 'started' | 'stopped'; port: number | null }) =>
        callback(payload)
      ipcRenderer.on(IpcChannel.Artifact_ServerStatus, listener)
      return () => {
        ipcRenderer.removeListener(IpcChannel.Artifact_ServerStatus, listener)
      }
    }
  },
  claudeCodePlugin: {
